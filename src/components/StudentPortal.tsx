...
import { Award, BookOpen, Calendar, FileSpreadsheet, LogOut } from 'lucide-react';
import { Grade, Lesson, Subject, User } from '../types.js';
...
type TabType = 'schedule' | 'gradebook';
...
export default function StudentPortal({ user, token, onLogout }: StudentPortalProps) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [scheduleRes, subjectsRes, gradebookRes] = await Promise.all([
        fetch('/api/schedule', { headers: { Authorization: `Bearer ${token}` } }),
        fetch('/api/subjects', { headers: { Authorization: `Bearer ${token}` } }),
        fetch('/api/gradebook', { headers: { Authorization: `Bearer ${token}` } })
      ]);

      const [scheduleData, subjectsData, gradebookData] = await Promise.all([
        scheduleRes.json(),
        subjectsRes.json(),
        gradebookRes.json()
      ]);
